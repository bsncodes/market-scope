...
- `/frontend` — Vite + React app (empty scaffold is fine for now).
...
- Migration files live under `backend/migrations/` — migrations are a
...
- Wire `npm run migrate:up` / `npm run migrate:down` in `backend/package.json`.
...
- **Source: an npm package** with India state/city reference data (e.g.
  ...
  Add it as a `backend` dependency, and have the seed script import it
...
- **Runner shape**: a single seed script (e.g. `backend/scripts/seed.js`,
